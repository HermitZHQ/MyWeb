...
// $response = $longLoan2;
    ...
    $startDataIndex = count($dataArr) - count($infoArr) - 5;
    $yearCount = $dataArr[$startDataIndex-1] - 1;
    ...
    if ($AverageEbitIncrease > 0.2) {
        $ebitAdjustedYear = 2;
    } elseif ($AverageEbitIncrease >= 0.05) {
        $ebitAdjustedYear = 1;
    } elseif ($AverageEbitIncrease >= 0) {
        $ebitAdjustedYear = 0;
    } else {
        $ebitAdjustedYear = -1;
    }
    ...
    } elseif ($averageAssetsLiabilitiesRatio >= 0.7) {
        $safeBoundAdjusted -= 0.15;
    } elseif ($averageAssetsLiabilitiesRatio >= 0.6) {
        $safeBoundAdjusted -= 0.1;
    } elseif ($averageAssetsLiabilitiesRatio >= 0.5) {
    ...
    $bearTheory1 = $bearTheory1Flag ? 1 : 0;
    $bearTheory2 = $bearTheory2Flag ? 1 : 0;
    $bearTheory3 = $bearTheory3Flag ? 1 : 0;
    $bearTheory4 = $bearTheory4Flag ? 1 : 0;
    $profitability = 0;
    if ($averageRoic > 0.2) {
        $profitability = 5;
    } elseif ($averageRoic > 0.15) {
        $profitability = 4;
    } elseif ($averageRoic > 0.1) {
        $profitability = 3;
    } elseif ($averageRoic > 0.05) {
        $profitability = 2;
    } elseif ($averageRoic > 0) {
        $profitability = 1;
    }
    ...
    $comment = "到".$latestYear."年
    $name($queryID)
    盈利能力：".$profitability."星级
    10年回本价格：$precisionAssessment10Value
    7年回本价格：$precisionAssessment7Value
    小熊1-4：$bearTheory1$bearTheory2$bearTheory3$bearTheory4";

    $query = "INSERT INTO $dbName.$tableName (id,name,value,tvalue,assessment7,assessment10,b1,b2,b3,b4,comment) ".
    "values('$queryID', '$name', '$value', '$precisionAssessment10Value', '$precisionAssessment7Value', '$precisionAssessment10Value', '$bearTheory1', '$bearTheory2', '$bearTheory3', '$bearTheory4', '$comment' )".
    " on duplicate key update name='$name', value='$value', tvalue='$precisionAssessment10Value', assessment7='$precisionAssessment7Value', assessment10='$precisionAssessment10Value', b1='$bearTheory1', b2='$bearTheory2', b3='$bearTheory3', b4='$bearTheory4', comment='$comment'";
...
$jsArr = array(
    ...
    "handleNum"=>$realCount,
    "totalTime"=>$elapsed,
    "averageTime"=>($realCount > 0 ? ($elapsed/(float)$realCount) : 0 )
);
echo json_encode($jsArr);
// echo "total handle:$realCount";
// echo "total handle:".$realCount."\naverage handle time is:".($elapsed/(float)$realCount);
